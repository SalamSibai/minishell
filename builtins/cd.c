...
/**
 * @brief this function get a string that contains the path specifed in the const var and it looks for it in it env sturcture if ts in there
 * and if found it start copying the rest of the path from the env
 * @param env A pointer to a structure (`t_env`) containing information about the shell environment.
 * @param var is the path_name to look for in the env sturcture mainly "HOME" OR "OLDPWD"
...
char	*get_env_path(t_list *env, const char *var, size_t size)
{
	...
	while (env && env->next)
	{
		...
		{
			len = ft_strlen(env->content) - (int)size;
			env_path = ft_calloc(1, len + 1);
			i = 0;
			j = 0;
			while (env_content[i])
			{
				if (i > (int)size)
					env_path[j++] = env_content[i];
				i++;
			}
			env_path[j] = '\0';
			return (env_path);
...
/**
 * @brief this function changes the path based on the option given from the cd function
 * if option is 0 itll go to the home path as 0 represent no arguments were given
 * else if option is 1 itll go to the oldpwd as 1 represent that the argument '-' were recieved
 * @param option a flag to change the path to HOME or OLDPWD
 * @param env A pointer to a structure (`t_env`) containing information about the shell environment.
 * @return it frees the env_path allocated for the path and then returns zero on succses and -1 if an error occured
 */
 ...
 * @param env A pointer to a structure (`t_env`) containing information about the shell environment.
 * @return Returns 0 on success or -1 if there was an error changing the directory.
 */
